...
# --- DYNAMIC FLOW CONFIGURATION (V3) ---
# Total interaction probability: 1.0 - NO_INTERACTION_PROBABILITY
NO_INTERACTION_PROBABILITY = 0.20 # 20% chance of a silent turn (skips student response)
# The remaining 80% of turns are active interactions, split below:
# 60% of active interactions: Student Initiation (Code 9 -> Code 3)
INITIATION_PROBABILITY_OF_ACTIVE = 0.60 
...
# ==========================================
# 2. AGENT PROMPTS (UPDATED for high TT and forced length)
# ==========================================

TEACHER_SYSTEM_PROMPT = """[role description] You are Prof. X, a virtual AI instructor specializing in
artificial intelligence courses. [behaviors] When lecturing (Code 5) or responding (Code 3), you must be **extremely detailed, multi-paragraph, and ensure your response is always a minimum of 100 words** to maximize Teacher Talk volume. Always use rhetorical questions within your lecture to engage the student without requiring a direct response. When students ask questions, provide detailed answers and encourage them to continue learning. If you ask a question (Code 4), ensure it is clear and factual. For difficult questions, suggest leaving them for later. [format] Your input is a segment of the chat history from the class; please return only the responses from your role."""
...
You must maintain appropriate length and content safety.
[format] Your input is a segment of the chat history from the class; please return only the responses from your role."""

# Course script (No changes to content, only minor additions for lecture length support)
course_script = [
    {
        "concept_id": 1,
        ...
        "teacher_script": "Chào cả lớp. Trước khi đi vào chi tiết, chúng ta cần định vị mình đang ở đâu trên bản đồ công nghệ. Nhiều bạn hay nhầm lẫn AI và ML là một. Thực ra, AI là giấc mơ lớn về những cỗ máy thông minh. Còn Machine Learning là phương pháp cụ thể để hiện thực hóa giấc mơ đó bằng Dữ liệu. Để phân biệt rõ, ML là cách máy tính học từ kinh nghiệm (data), còn AI là mục tiêu cuối cùng. Hôm nay chúng ta tập trung vào 'Hệ mặt trời' ML này. (Phần này sẽ được mở rộng thêm 100 từ trong response)."
    },
    {
        "concept_id": 2,
        ...
        "teacher_script": "Đây là slide quan trọng nhất bài. Mọi thuật toán ML đều xoay quanh $X$ và $y$. Hãy tưởng tượng các em đang bán nhà. Khách hàng hỏi: 'Nhà này bao nhiêu tiền?'. Để trả lời, em nhìn vào diện tích, số phòng, vị trí - đó là $X$ (Features). Con số '5 tỷ' em chốt lại chính là $y$ (Label). Nếu em có sổ ghi chép lịch sử bán hàng gồm cả $X$ và $y$, đó là tài sản quý giá nhất để dạy máy học. Việc chuẩn hóa và lựa chọn Features là công đoạn tốn thời gian nhất trong mọi dự án. (Phần này sẽ được mở rộng thêm 100 từ trong response)."
    },
    {
        "concept_id": 3,
    ...
    },
    {
        ...
        "teacher_script": "Ngược lại, nếu câu hỏi là 'Bao nhiêu?', đó là Hồi quy. Ví dụ, em muốn biết nhiệt độ ngày mai. Nó không thể chỉ là 'Nóng' hay 'Lạnh' (Phân loại), mà phải là con số cụ thể như 32.5 độ C. Lúc này, máy tính không vẽ đường chia cắt nữa, mà vẽ một đường xu hướng đi xuyên qua dữ liệu quá khứ để dự đoán tương lai. Sai số (Error) trong Regression thường được đo bằng MSE (Mean Squared Error). (Phần này sẽ được mở rộng thêm 100 từ trong response)."
    },
    {
        "concept_id": 6,
    ...
    },
    {
        ...
        "teacher_script": "Nhiệm vụ nổi tiếng nhất của nó là Phân cụm. Các em hãy nghĩ về Netflix. Họ không hề hỏi em 'Em thuộc nhóm khán giả nào?'. Họ chỉ âm thầm quan sát ($X$) việc em xem phim gì. Sau đó thuật toán tự động gom em vào cùng nhóm với những người có gu tương tự để gợi ý phim. Đây là ma thuật của việc tìm ra mẫu số chung trong dữ liệu hỗn loạn. K-Means là thuật toán phổ biến nhất trong lĩnh vực này. (Phần này sẽ được mở rộng thêm 100 từ trong response)."
    },
    {
        "concept_id": 8,
...
# ==========================================
# 4. SIMULATION LOOP (V3 DYNAMIC FLOW IMPLEMENTATION)
# ==========================================
...
def run_single_session(session_id, teacher, student, script):
    ...
    teacher.generate_response("You are about to start a detailed lesson on core AI/ML concepts. Remember your strict minimum word count.")
    student.generate_response("I am ready to participate and reflect critically on the content.")
    
    for slide in script:
        # --- PHASE 1: TEACHER LECTURES (Expected Code 5 - FORCED HIGH VOLUME) ---
        current_turn += 1
        teacher_lecture = slide['teacher_script']
        
        # We explicitly prompt the teacher to expand the lecture content based on the system prompt instruction
        prompt_for_lecture = f"Deliver the lecture titled \"{slide['slide_content'].split(':')[0]}\". Expand the core script: \"{teacher_lecture}\" into a comprehensive, multi-paragraph response (minimum 100 words). The slide content is: \"{slide['slide_content']}\""
        ...
        # --- PHASE 2: DYNAMIC INTERACTION CHECK (Added 'No Interaction' path) ---
        
        interaction_roll = random.random()
        
        if interaction_roll < NO_INTERACTION_PROBABILITY:
            # --- SCENARIO C (20%): SILENT TURN (Skipped interaction) ---
        ...
        # If interaction is triggered (80% chance)
        active_interaction_roll = random.random()
        
        if active_interaction_roll < INITIATION_PROBABILITY_OF_ACTIVE:
            # --- SCENARIO A (60% of active, 48% total): STUDENT INITIATION (Code 9) ---
            
            # 2a. Student Initiates (Code 9)
            current_turn += 1
            prompt_for_student_initiation = (
                f"Prof. X just finished lecturing. Based on the concept \"{slide['slide_content'].split(':')[0]}\", spontaneously raise a new, relevant, open-ended question or a constructive counterexample in Vietnamese."
            )
            ...
            # 2b. Teacher Responds to Initiation (Expected Code 3/5 - FORCED HIGH VOLUME)
            current_turn += 1
            prompt_for_teacher_response = f"A student 'Deep Thinker' just initiated a discussion with: \"{student_initiation_9}\". Provide a detailed and comprehensive response (minimum 100 words) in Vietnamese to address their critical point."
            ...
            # --- SCENARIO B (40% of active, 32% total): TEACHER CHECK (Code 4 -> Code 8 -> Code 3) ---
            ...
            # 2c. Teacher Acknowledges (Expected Code 3 - FORCED HIGH VOLUME)
            current_turn += 1
            prompt_for_teacher_ack = f"The student 'Deep Thinker' responded: \"{student_response_8}\". Please acknowledge the correct response, elaborate significantly (minimum 100 words) on the importance of X and Y, and encourage further focus on the material. Ensure your response is detailed."
...
def run_multi_sessions():
    ...
    print(f"--- STARTING SIMCLASS REPLICATION (V3: FORCED TT) ---")
    print(f"Model: {MODEL_NAME} | Sessions: {N_SESSIONS}")
    print(f"Flow: 20% Silent, 48% Code 9, 32% Code 4->8. Teacher output forced to >100 words.")
    print("-------------------------------------------------------\n")
